...
class BasicAdapter(
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        Log.i("item","seection"+listdata[position].iscChecked)
        ...
        holder.itemView.selectionView.setOnCheckedChangeListener { _, b ->
            when (ctx) {
                is Fragment__A -> mListenerAdapter.onItemSelected(listdata[position].value,b)
                is Fragment__B -> mListenerAdapter.onItemBSelected(listdata[position].value,b)
                is Fragment__C -> mListenerAdapter.onItemCSelected(listdata[position].value,b)
            }
